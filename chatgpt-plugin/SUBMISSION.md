...
| `audit_public_site` | Fetches one authorized public site seed, then reads bounded same-origin `robots.txt` plus XML Sitemap, RSS 2.0, Atom 1.0, and plain-text sitemap representations to audit at most eight same-origin eligible `text/html` pages. It respects applicable robots rules, rejects private/reserved/custom-port and cross-origin destinations/redirects, does not follow HTML links, and does not run site-wide link checks. Eligible page HTML may be sent to `https://html5.validator.nu/`; SEO/accessibility and JSON-LD checks run inside the Worker. | `readOnlyHint: true`, `openWorldHint: true`, `destructiveHint: false` |
